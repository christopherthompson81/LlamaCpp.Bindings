...
- [x] **Tooltip provider context** — Avalonia's attached `ToolTip.Tip` is used on ~30+ interactive elements across the app; themed globally via `Style Selector="ToolTip"` in `Controls.axaml` (dark pill, `RadiusSm`, `FontSizeXs`).
...
- [x] **Attachment list modal** — `Views/PendingAttachmentsDialog.cs` — wrap-panel gallery with 140×140 tiles, per-tile Remove + bottom Clear-all. The compose-bar strip caps at 4 tiles; when more are queued, a `+N` overflow tile opens the modal. Live-synced to `PendingAttachments` via `CollectionChanged`; dialog auto-closes when the queue empties.
...
- [x] **Tooltip** — `Style Selector="ToolTip"` sets the dark pill, small font, and rounded corners; attached `ToolTip.Tip` gives both label + expository text to the arcane sampler controls (dynatemp, top-k/p, min-p, typical, top-n-σ, XTC).

...
- [~] **Mermaid diagrams** — deferred but no longer blocked by a "DOM required" handwave. Plan tracked in `docs/diagram_rendering_investigation.md`: hand-rolled parser + Sugiyama layered layout + orthogonal edge routing, rendering to an Avalonia `Canvas` with labels measured via `FormattedText`. V1 scope is flowchart only (`graph TD/LR/RL/BT`, `flowchart TD/...`, rectangle/rounded/stadium/circle/rhombus nodes, `-->` / `---` / `-.->` / `==>` edges with optional labels). Other graph-shaped types (state, class, ER, C4) re-use the layout engine as different node/edge template sets; linear types (sequence, gantt, …) get their own lane-based renderers.
